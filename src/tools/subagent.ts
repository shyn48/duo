...
export function registerSubagentTools(server: McpServer) {
  server.tool(
    "duo_subagent_spawn",
    "Spawn a sub-agent for an AI-assigned task. Returns a structured prompt that the mother agent can use to spawn via whatever mechanism is available (OpenClaw sessions_spawn, Claude Code Task, etc.).",
    {
    ...
    async ({ taskId, description, prompt, files, dependencies, model, background }) => {
      ...
      contextParts.push(
        ...
        `When you finish, you MUST write your results to: ${resultPath}`,
        ...
        "issues:",
        "  - Any concerns or questions for the orchestrator (or empty)",
        "completedAt: <ISO timestamp>",
        "```",
        "This file is the ONLY communication channel back to the mother agent.",
        "Do not rely on conversation text for handoff.",
      );
  ...
  server.tool(
    "duo_subagent_read_result",
    "Read the structured result written by a sub-agent after it finishes. Sub-agents write to .duo/subagent-results/{taskId}.md when done. Call this to retrieve and integrate their work.",
    {
  ...
  server.tool(
    "duo_subagent_mark_complete",
    "Mark a sub-agent task as complete after reviewing its result. Updates session state, records files changed, and auto-checkpoints. Call this after duo_subagent_read_result once you've reviewed the work.",
    {
    ...
    async ({ taskId, filesChanged, summary, issues, status }) => {
      ...
      if (!task) {
        return {
          content: [{ type: "text" as const, text: `Task [${taskId}] not found on board.` }],
          isError: true,
        };
      }
      ...
      await state.logChat(
        "system",
        "event",
        `Sub-agent [${taskId}] ${status} — ${filesChanged.length} files changed`,
        taskId,
      );
