...
export class DuoWatcher {
  ...
  async start(): Promise<void> {
    ...
    try {
      const watcher = watch(
        ...
        (eventType, filename) => {
          if (!filename) return;
          this.handleChange(eventType as "rename" | "change", filename);
        },
      );
  ...
  private async watchDirectoriesRecursive(dir: string): Promise<void> {
    const relDir = relative(this.projectRoot, dir);
    if (this.shouldIgnore(relDir)) return;

    try {
      const watcher = watch(dir, (eventType, filename) => {
        if (!filename) return;
        const relPath = join(relDir, filename);
        this.handleChange(eventType as "rename" | "change", relPath);
      });
      this.watchers.push(watcher);
