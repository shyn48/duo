 ...
 */
export async function ensureDocsDir(stateDir: string): Promise<string> {
...
export function registerDocumentTools(server: McpServer) {
  server.tool(
    "duo_document_save",
    "Save a document to .duo/docs/ for project documentation. Auto-generates filename with phase and date.",
    {
      ...
      try {
        ...
        return {
          content: [
            {
              type: "text" as const,
              text: `Error saving document: ${(e as Error).message}`,
            },
          ],
          isError: true,
        };
