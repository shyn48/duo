...
- `GET /api/snapshot` → `{ snapshot, reviewTargets, decisions }`;
...
Ranking candidates are Go-internal inputs. They are not serialized as part of the public snapshot contract; the browser receives only the normalized snapshot, ranked review targets, and recorded decisions.
