...
export function registerRecoverTools(server: McpServer) {
  server.tool(
    "duo_recover_session",
    "Recover a Duo session from the most recent memory checkpoint. Auto-detects checkpoints in .duo/memory/. Use this after a crash or context loss.",
    {},
    async () => {
      ...
      for (const sub of checkpoint.subagents) {
        const alreadyTracked = existingSubagents.some(
          (s) => s.taskId === sub.taskId && s.spawnedAt === sub.spawnedAt,
        );
        if (!alreadyTracked) {
      ...
      const doneTasks = checkpoint.tasks.filter(
        (t) => t.status === "done",
      ).length;
